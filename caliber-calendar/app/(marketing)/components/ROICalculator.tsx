...
function SliderInput({
  ...
  prefix, suffix, onChange, formatDisplay,
}: SliderInputProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <input
        ...
        style={{
          background: `linear-gradient(to right, #059669 0%, #059669 ${pct}%, rgba(255,255,255,0.10) ${pct}%, rgba(255,255,255,0.10) 100%)`,
        }}
      />
...
function MetricCard({
  ...
  return (
    <div className={`rounded-xl p-5 text-center ${
      accent
        ? 'bg-emerald-600/[0.12] border border-emerald-600/30'
        : 'bg-white/[0.05] border border-white/[0.08]'
    }`}>
      <Icon className={`w-5 h-5 mx-auto mb-2 ${accent ? 'text-emerald-400' : 'text-slate-400'}`} />
      <div className={`text-xl font-extrabold tabular-nums mb-0.5 ${accent ? 'text-emerald-400' : 'text-white'}`}>
        {value}
      </div>
...
export default function ROICalculator() {
  ...
  const update = useCallback(<K extends keyof CalcState>(key: K, val: number) => {
  ...
  const roi                = monthlyInvestment > 0
                               ? ((netProfit / monthlyInvestment) * 100)
                               : 0
  const isPositive = netProfit >= 0

  // helper for large number display on slider labels
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-slate-400 text-xl max-w-xl mx-auto">
            Plug in your variables. See exactly what Caliber Calendar delivers.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-start">
          ...
          <div className="bg-[#0E243A] border border-white/[0.08] rounded-2xl p-8 space-y-10">
            ...
            <SliderInput
              id={`${uid}-commission`}
              label="Avg. Commission per Closed Case"
            ...
            <SliderInput
              id={`${uid}-cost`}
              label="Cost per Show Appointment"
              value={state.costPerShow}
              min={300} max={1500} step={25}
              prefix="$"
              onChange={(v) => update('costPerShow', v)}
            />

            {/* Disclaimer inside inputs */}
            <div className="flex items-start gap-2 text-slate-600 text-xs bg-white/[0.03] rounded-lg p-3 border border-white/[0.05]">
              ...
              <span>
                <strong className="text-slate-500">Illustration only.</strong>{' '}
                Results depend on your closing ability, case mix, market conditions, and prospect quality.
          ...
          <div className="space-y-5 lg:sticky lg:top-24">
            ...
            <div className={`rounded-2xl p-8 text-center border transition-all duration-300 ${
              isPositive
                ? 'bg-emerald-600/10 border-emerald-600/35'
                : 'bg-red-600/10 border-red-600/35'
            }`}>
              <div className="text-slate-400 text-xs font-semibold uppercase tracking-[0.15em] mb-3">
                Est. Net Monthly Profit
              </div>
              <div className={`text-6xl sm:text-7xl font-extrabold tabular-nums leading-none mb-2 transition-colors duration-200 ${
              ...
              {/* ROI badge */}
              <div className={`inline-flex items-center gap-1.5 mt-4 px-3 py-1.5 rounded-full text-xs font-semibold border ${
                isPositive
            ...
            <a
              href="#book-call"
              className="flex items-center justify-center gap-2 w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 px-6 rounded-xl transition-all duration-200 hover:shadow-emerald-glow hover:-translate-y-0.5 active:scale-[0.98] group text-base"
            >
            ...
            <p className="text-center text-slate-600 text-xs">
              Free discovery call · No commitment · Qualified advisors only
            </p>
          </div>
        </div>
      </div>
