    ...
    <section className="relative min-h-screen flex items-center overflow-hidden pt-16">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28 lg:py-36">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-14">
          <Link
            href="#book-call"
            className="inline-flex items-center justify-center gap-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-9 py-4.5 rounded-xl text-lg transition-all duration-200 hover:shadow-emerald-glow hover:-translate-y-0.5 active:scale-[0.98] group"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
          {STATS.map(({ value, label, sub }) => (
            <div
              key={label}
              className="bg-white/[0.04] hover:bg-white/[0.06] border border-white/[0.08] rounded-2xl p-5 lg:p-6 transition-colors duration-200 group"
            >
              <div className="text-3xl lg:text-4xl font-extrabold text-emerald-400 mb-1.5 tabular-nums">
                {value}
              </div>
        ...
        <div className="mt-12 flex items-center gap-3 text-slate-400 text-sm">
          <div className="flex -space-x-2">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-600 to-emerald-800 border-2 border-[#0A192F] flex items-center justify-center text-xs font-bold text-white"
              >
