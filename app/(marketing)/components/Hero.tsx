...
// Cumulative revenue per appointment
let _c = 0
...
// ── Round-rect helper (avoids browser compat issues) ────────
...
function PipelineCanvas() {
  ...
  const raf       = useRef(0)
  const prog      = useRef(0)   // curve reveal 0→1
  const tick      = useRef(0)
  ...
  const draw = useCallback(() => {
    ...
    tick.current  += 0.016
    prog.current   = Math.min(1, prog.current + 0.005) // ~3s draw-in
    ...
    ctx.textAlign = 'right'
    ctx.font = `500 ${Math.max(9, W * 0.022)}px Inter, system-ui, sans-serif`
    yTicks.forEach(v => {
      const gy = PT + CH - (v / MAX_REV) * CH
      ctx.fillStyle = 'rgba(52,211,153,0.38)'
      ctx.fillText(v === 0 ? '$0' : `$${v / 1000}K`, PL - 7, gy + 4)
    })

    // ── Build curve points from revealed appointments ─
    type Pt = { x: number; y: number }
    const cpts: Pt[] = [{ x: PL, y: PT + CH }]
    APPTS.forEach((a, i) => {
      if (a.t > p) return
      cpts.push({ x: PL + a.t * CW, y: PT + CH - (CUM[i] / MAX_REV) * CH })
    })

    // ── Appointment dots + verticals ─────────────────
    APPTS.forEach((a, i) => {
      ...
      const rev   = a.t <= p
      const hover = rev && MI && Math.abs(MX - ax / W) < 0.055
      ...
      if (a.show) {
        ...
        if (rev) {
          ...
          ctx.lineWidth = 1
          ctx.beginPath(); ctx.moveTo(ax, baseY); ctx.lineTo(ax, revY); ctx.stroke()
          ctx.restore()
        }
        ...
        ctx.beginPath(); ctx.arc(ax, baseY, dr, 0, Math.PI * 2)
        ...
        // Outer ring on hover
        if (hover) {
          ctx.beginPath(); ctx.arc(ax, baseY, dr + 5, 0, Math.PI * 2)
          ctx.strokeStyle = 'rgba(52,211,153,0.22)'; ctx.lineWidth = 1.5; ctx.stroke()
        }

        // Commission badge
        if (rev && (hover || p > a.t + 0.14)) {
        ...
        if (hover) {
          const cw = 162, ch = 76
          const cx = Math.min(ax + 14, PL + CW - cw - 4)
          const cy = Math.max(PT + 8, revY - ch - 8)
          rrect(ctx, cx, cy, cw, ch, 7)
          ctx.fillStyle = 'rgba(7,20,38,0.97)'; ctx.fill()
          ctx.strokeStyle = 'rgba(52,211,153,0.38)'; ctx.lineWidth = 1; ctx.stroke()

          ctx.textAlign = 'left'
          ctx.fillStyle = 'rgba(255,255,255,0.93)'
          ctx.font = `bold ${Math.max(10, W * 0.023)}px Inter, sans-serif`
          ctx.fillText(`${a.name}, ${a.age}`, cx + 10, cy + 22)

          ctx.fillStyle = 'rgba(148,163,184,0.72)'
          ctx.font = `${Math.max(9, W * 0.019)}px Inter, sans-serif`
          ctx.fillText(`${a.assets} investable`, cx + 10, cy + 40)

          ctx.fillStyle = '#34d399'
          ctx.font = `bold ${Math.max(9, W * 0.02)}px Inter, sans-serif`
          ctx.fillText(`✓  Showed  ·  +$${(a.comm / 1000).toFixed(1)}K`, cx + 10, cy + 60)
        }
      } else {
        // No-show — muted
        ctx.beginPath(); ctx.arc(ax, baseY, rev ? 3.5 : 2, 0, Math.PI * 2)
        ctx.fillStyle = rev ? 'rgba(100,116,139,0.32)' : 'rgba(100,116,139,0.1)'; ctx.fill()

        if (rev && MI && Math.abs(MX - ax / W) < 0.04) {
          const cx = Math.min(ax + 10, PL + CW - 156)
          const cy = PT + CH * 0.55
          rrect(ctx, cx, cy, 150, 48, 6)
          ctx.fillStyle = 'rgba(7,20,38,0.95)'; ctx.fill()
          ctx.strokeStyle = 'rgba(100,116,139,0.28)'; ctx.lineWidth = 1; ctx.stroke()
          ctx.textAlign = 'left'
          ctx.fillStyle = 'rgba(255,255,255,0.8)'
          ctx.font = `bold ${W * 0.021}px Inter, sans-serif`
          ctx.fillText(`${a.name}, ${a.age}`, cx + 9, cy + 19)
          ctx.fillStyle = 'rgba(148,163,184,0.6)'
          ctx.font = `${W * 0.018}px Inter, sans-serif`
          ctx.fillText('No-show · $0 charged', cx + 9, cy + 36)
        }
      }
    })
    ...
    if (cpts.length >= 2) {
      ...
      cpts.forEach(pt => ctx.lineTo(pt.x, pt.y))
      ...
      ctx.shadowColor = '#059669'; ctx.shadowBlur = 16; ctx.stroke()
      ctx.shadowBlur = 0

      // Live pulse dot at reveal edge
      if (p < 0.98) {
        const edge = cpts[cpts.length - 1]
        const pulse = 0.6 + Math.sin(TK * 5) * 0.4
        ctx.beginPath(); ctx.arc(edge.x, edge.y, 5.5, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(52,211,153,${pulse})`
        ctx.shadowColor = '#34d399'; ctx.shadowBlur = 22; ctx.fill(); ctx.shadowBlur = 0
      }
    }
    ...
    ptcl.current = ptcl.current.filter(pt => pt.life < pt.maxLife)
    ptcl.current.forEach(pt => {
      pt.x += pt.vx; pt.y += pt.vy; pt.life++
      const a = Math.sin((pt.life / pt.maxLife) * Math.PI) * 0.28
      ctx.beginPath(); ctx.arc(pt.x, pt.y, pt.r, 0, Math.PI * 2)
      ctx.fillStyle = `rgba(52,211,153,${a})`; ctx.fill()
    })

    // ── Revenue counter ──────────────────────────────
    let revNow = 0
    APPTS.forEach((a, i) => { if (a.show && a.t <= p) revNow = CUM[i] })

    ctx.textAlign = 'right'
    ctx.fillStyle = 'rgba(52,211,153,0.50)'
    ctx.font = `500 ${Math.max(9, W * 0.019)}px Inter, sans-serif`
    ctx.fillText('cumulative this month', PL + CW, PT - 24)

    ctx.fillStyle = 'rgba(255,255,255,0.94)'
    ctx.font = `bold ${Math.max(14, W * 0.052)}px Inter, sans-serif`
    ctx.fillText(`$${revNow.toLocaleString()}`, PL + CW, PT - 6)

    // ── X-axis label ─────────────────────────────────
    ctx.textAlign = 'center'
    ctx.fillStyle = 'rgba(148,163,184,0.28)'
    ctx.font = `500 ${Math.max(8, W * 0.016)}px Inter, sans-serif`
    ctx.fillText('QUALIFIED APPOINTMENTS  →', PL + CW / 2, H - H * 0.055)

    // ── Hint text ────────────────────────────────────
    if (!MI && p > 0.6) {
      ctx.fillStyle = 'rgba(148,163,184,0.22)'
      ctx.font = `${Math.max(8, W * 0.017)}px Inter, sans-serif`
      ctx.fillText('hover to explore each appointment', PL + CW / 2, PT + CH / 2 + 4)
    }
  ...
  useEffect(() => {
    ...
    const onMove  = (e: MouseEvent) => {
      ...
      mouse.current = { x: (e.clientX - r.left) / r.width, y: (e.clientY - r.top) / r.height, inside: true }
    }
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 lg:pt-24">
        <div className="lg:grid lg:grid-cols-[1fr_1.15fr] lg:gap-10 xl:gap-16 lg:items-center">
          ...
          <div className="pb-10 lg:pb-0">
            ...
            <div className="mb-8 sm:mb-10">
              <div className="relative inline-flex">
                <div
                  ...
                  style={{ background: 'radial-gradient(ellipse at 40% 50%, rgba(5,150,105,0.20) 0%, transparent 75%)', filter: 'blur(18px)' }}
                />
                <Image
                  src="/logo-light.svg"
                  alt="Caliber Calendar"
                  width={520} height={136} priority
                  className="relative h-[68px] sm:h-[84px] lg:h-[96px] w-auto max-w-[300px] sm:max-w-[420px] lg:max-w-[500px]"
                  style={{ filter: 'drop-shadow(0 0 28px rgba(5,150,105,0.38)) drop-shadow(0 5px 22px rgba(0,0,0,0.60))' }}
                />
              </div>
            ...
            {/* Badges */}
            <div className="flex flex-wrap items-center gap-2.5 mb-7 sm:mb-8">
              <div className="inline-flex items-center gap-2 bg-emerald-600/[0.10] border border-emerald-500/28 rounded-full px-3.5 py-2">
            ...
            {/* Headline */}
            <h1 className="text-[2.1rem] sm:text-4xl lg:text-[3.25rem] font-extrabold leading-[1.1] tracking-tight mb-5 lg:mb-6">
              <span className="text-white">Qualified $500K+ Appointments. </span>
              <span className="gradient-text">Pay Only When They Show.</span>
            </h1>

            {/* Sub */}
            <p className="text-base sm:text-lg text-slate-300 leading-relaxed mb-7 lg:mb-8 max-w-lg">
            ...
            {/* CTAs */}
            <div className="flex flex-col sm:flex-row sm:items-center gap-y-4 gap-x-5 mb-7">
              <Link
                href="#book-call"
          ...
          {/* ── Right: interactive canvas (desktop only) ── */}
          <div className="hidden lg:block relative rounded-2xl overflow-hidden border border-white/[0.06] bg-[#060E1B]/60" style={{ height: '500px' }}>
            {/* Subtle inner glow */}
            <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 80% 60% at 50% 80%, rgba(5,150,105,0.07) 0%, transparent 70%)' }} />
            <PipelineCanvas />
            {/* Corner label */}
            <div className="absolute top-3 left-4 text-[10px] font-semibold text-slate-600 tracking-[0.12em] uppercase pointer-events-none">
              Live Pipeline Model
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mt-10 lg:mt-12">
          {STATS.map(({ value, label, sub }) => (
            ...
            >
              <div className="text-2xl lg:text-[2rem] font-extrabold text-emerald-400 mb-1 tabular-nums leading-none">{value}</div>
        ...
        {/* ── Social proof ──────────────────────────────────── */}
        <div className="mt-7 flex flex-wrap items-center gap-x-3 gap-y-2 text-slate-500 text-xs sm:text-sm">
          <div className="flex -space-x-2 shrink-0">
            {['A','B','C','D','E'].map(l => (
              <div key={l} className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-emerald-700 to-emerald-900 border-2 border-[#0A192F] flex items-center justify-center text-[9px] sm:text-[10px] font-bold text-white">
                {l}
              </div>
            ))}
          </div>
